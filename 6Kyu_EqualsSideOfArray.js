...
function findEvenIndex(arr) {
    ...
    // loop over the array again and remove one number at a time from left to right
    for (var i = 0; i < arr.length; i++) {
        sum -= arr[i];
        // do the two values match now? if yes return the index
        if (leftSum === sum) {
            return i;
        }
        // if not add the value to our left sum and continue looping
        leftSum += arr[i];
    }
    // once the loop finishes, if it failed finding a result then return -1
    return -1
}